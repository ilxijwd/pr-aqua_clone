<template>
  <div class="px-3 space-y-8 md:space-y-12 lg:space-y-16 max-w-6xl mx-auto">
    <h1 class="text-center font-bold text-lg md:text-2xl lg:text-4xl">
      {{ $t('pages.products.deferum-05-semi-auto.title') }}
    </h1>
    <div
    ...
    >
      <h2 class="font-bold text-center text-base md:text-lg lg:text-2xl">
        {{ $t('pages.products.deferum-05-semi-auto.tech-process.title') }}
      </h2>
      <div>
        {{ seoText1 }}
      </div>
      <h3 class="font-bold text-center lg:text-2xl">
        {{ $t('pages.products.deferum-05-semi-auto.deferum-05.title') }}
      </h3>
      <div>
        {{ seoText2 }}
      </div>
    ...
    <div class="text-center font-bold md:text-xl lg:text-4xl">
      {{ $t('pages.products.deferum-05-semi-auto.specs') }}
    </div>
    <table class="info-table">
      <tbody>
        <tr v-for="(row, row_index) in $t('pages.products.deferum-05-semi-auto.table')" :key="`table-row-${row_index}`">
...
export default Vue.extend({
  async asyncData({ app }) {
    ...
    const deferum05SemiAutoPage = await app.$strapi.find('deferum-05-semi-auto-page')
    return {
      seoText1: deferum05SemiAutoPage[`seoText1_${lang}`],
      seoText2: deferum05SemiAutoPage[`seoText2_${lang}`],
      title: deferum05SemiAutoPage.pageMetaTags[`title_${lang}`],
      description: deferum05SemiAutoPage.pageMetaTags[`description_${lang}`],
    }
  },
  head() {
    return {
